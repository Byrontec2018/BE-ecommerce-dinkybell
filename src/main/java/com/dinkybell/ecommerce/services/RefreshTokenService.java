...
import com.dinkybell.ecommerce.repositories.RefreshTokenRepository;
...
public class RefreshTokenService {
    ...
    public RefreshToken createRefreshToken(Long userId, HttpServletRequest request) {
        ...
        if (existingToken.isPresent()) {
            ...
            logger.debug("Reusing existing valid refresh token for user {} on same device", userId);
            return updatedToken;
        }
            ...
            if (activeTokenCount >= maxTokensPerUser) {
                ...
                refreshTokenRepository.findOldestActiveTokenByUserId(userId, Instant.now())
                    .ifPresent(oldestToken -> {
                        ...
                        logger.debug("Revoked oldest token for user {} due to limit", userId);
                    });
            }
        }
        ...
        logger.debug("Created new refresh token for user {} on new device", userId);
    ...
    public String refreshAccessToken(String token) {
        ...
        logger.debug("Generated new access token for user {}", refreshToken.getUserId());
    ...
    public void revokeRefreshToken(String token) {
        ...
        refreshTokenRepository.save(refreshToken);
        logger.debug("Revoked refresh token for user {}", refreshToken.getUserId());
    }
    ...
    public void revokeOtherTokens(String token) {
        ...
        logger.debug("Revoked {} other tokens for user {}", revokedCount, refreshToken.getUserId());
    }
    ...
    public int purgeExpiredTokens() {
        ...
        logger.debug("Purged {} expired or revoked refresh tokens", deleted);
        return deleted;
    }
    ...
    private String extractDeviceInfo(HttpServletRequest request) {
        ...
        logger.debug("Generated device fingerprint: {}", deviceFingerprint);
