    ...
    public static String resolveKey(String limiterName) {
        try {
            ...
            if (auth != null && auth.isAuthenticated() && !auth.getName().equals("anonymousUser")) {
                ...
                log.debug("Rate limiting key for authenticated user: {}", keyBuilder.toString());
            } else {
                ...
                log.debug("Rate limiting key for anonymous user: {}", keyBuilder.toString());
            }
            ...
            log.warn("Could not resolve rate limiting key, using fallback: {}", fallbackKey, e);
            return fallbackKey;
        }
    }
