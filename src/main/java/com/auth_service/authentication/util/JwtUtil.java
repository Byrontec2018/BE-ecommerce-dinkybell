package com.auth_service.authentication.util;
...
import com.auth_service.authentication.service.JwtKeyProvider;
 ...
 * (user email) - User ID - Roles - Issuer (auth-service-app) - Issued at timestamp - Expiration
...
public class JwtUtil {    
     ...
     * expiration timestamp - iss: issuer (auth-service-app) - jti: unique token ID
     * 
    ...
    public String generateToken(UserAuthentication user) {
        ...
        return Jwts.builder().setSubject(user.getEmail()).claim("userId", user.getId()) // Include user ID for easy identification
                ...
                .setIssuer("auth-service-app") // Application identifier
